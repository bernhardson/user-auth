...
func newTestDB(t *testing.T) *sql.DB {
	...
	// in one db.Exec() call.
	db, err := sql.Open("mysql", "test_web:pass@/test_stub?parseTime=true&multiStatements=true")
	if err != nil {
		t.Fatal(err)
	}

	// Read the setup SQL script from file and execute the statements.
	script, err := os.ReadFile("./test_data/setup.sql")
	if err != nil {
		t.Fatal(err)
	}
	...
	t.Cleanup(func() {
		script, err := os.ReadFile("./test_data/teardown.sql")
		if err != nil {
			t.Fatal(err)
		}
		_, err = db.Exec(string(script))
